	...
	return (
		<Fragment>
			<Row>
				<Col>
					<FormGroup>
						<CanCall action='ELECTIONS_ADD_PARTY'>
							...
							<Card className={`mx-25`} key={party.id} style={{flex:'0 0 275px'}}>
								<CardHeader>
									...
									<UncontrolledDropdown>
										...
										<DropdownMenu right>
											<CanCall action='ELECTIONS_EDIT_PARTY'>
											...
											<CanCall action='ELECTIONS_DELETE_PARTY'>
								...
								<CardBody>
									<CanCall action='ELECTIONS_ADD_CANDIDATE'>
										...
										return (
											<Fragment key={candidate.id}>
												<Row
													key={candidate.id}
													onClick={() => {
														if (ability.can('call', 'ELECTIONS_EDIT_CANDIDATE')) { _editCandidateModal(candidate) }
													}}
